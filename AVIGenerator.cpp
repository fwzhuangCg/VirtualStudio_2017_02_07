// AVIGenerator.cpp: implementation of the AVIGenerator class.
//
//////////////////////////////////////////////////////////////////////
...
AVIGenerator::AVIGenerator(LPCTSTR sFileName/*, CView* pView*/, DWORD dwRate, int width, int height)
: m_sFile(sFileName), m_dwRate(dwRate),
m_pAVIFile(NULL), m_pStream(NULL), m_pStreamCompressed(NULL)
{
		MakeExtAvi();
		SetBitmapHeader(/*pView*/width, height);
}
#endif

AVIGenerator::AVIGenerator(LPCTSTR sFileName, LPBITMAPINFOHEADER lpbih, DWORD dwRate)
: m_sFile(sFileName), m_dwRate(dwRate),
m_pAVIFile(NULL), m_pStream(NULL), m_pStreamCompressed(NULL)
{
		MakeExtAvi();
...
#ifdef _AVIGENERATOR_USE_MFC
void AVIGenerator::SetBitmapHeader(/*CView *pView*/int width, int height)
{
	//ASSERT_VALID(pView);
...
HRESULT AVIGenerator::InitEngine()
{
...
HRESULT AVIGenerator::AddFrame(BYTE *bmBits)
{
	HRESULT hr;

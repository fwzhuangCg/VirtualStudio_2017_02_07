...
int Camera::viewport_width_ = 500;
int Camera::viewport_height_ = 300;
...
QQuaternion Camera::calcRotation( const QVector3D& vFrom, const QVector3D& vTo )
{
	QVector3D vPart;
	float fDot = QVector3D::dotProduct(vFrom, vTo);
	vPart = QVector3D::crossProduct(vFrom, vTo);

	return QQuaternion(fDot, vPart);
}
...
void Camera::rotate(const QPoint& prevPos, const QPoint& curPos)
{
	...
	QQuaternion newRot = calcRotation(from, to);

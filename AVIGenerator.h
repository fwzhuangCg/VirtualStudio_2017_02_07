// AVIGenerator.h: interface for the AVIGenerator class.
//
...
public:
	...
	AVIGenerator(LPCTSTR sFileName/*, CView* pView*/, DWORD dwRate, int width, int height);
#endif
	//! Inplace constructor with BITMAPINFOHEADER
	AVIGenerator(LPCTSTR sFileName, LPBITMAPINFOHEADER lpbih, DWORD dwRate);
	~AVIGenerator();
	//@}
	...
	*/
	...
	void SetBitmapHeader(/*CView* pView*/int width, int height);
	...
	void SetFileName(const char * _sFileName)							{	m_sFile=_sFileName; /*MakeExtAvi();*/};
